                   ...
                   _In_ [[maybe_unused]] int           nShowCmd)
{
    ...
    bool         is_fullscreen     = ini.GetBool("System", "FullScreen");
    const float2 screen_size       = ini.GetFloat2("System", "ScreenSize", {WINDOW_W, WINDOW_H});
    const auto   title_name        = ini.GetString("System", "Title", "BaseProject2026");
    ...
    const bool   use_utf8_mode     = ini.GetBool("DxLib", "UseUTF8", false);
    ...
    if(ini.GetBool("System", "CheckFullScreen")) {
        int result = MessageBox(NULL,
                                TEXT("フルスクリーンモードで起動しますか？\n「いいえ」を選ぶとウィンドウモードになります。"),
                                TEXT("起動モード選択"),
    ...
    if(!is_fullscreen)    // GPU位置固定するため、初期化後行う必要がある
    {
