...
public:
    ...
    const matrix& GetViewMatrix() { return mat_view_; }
    const matrix& GetProjectionMatrix() const { return mat_proj_; }
    const matrix& GetProjectionMatrix() { return mat_proj_; }

private:
    Status<CameraBit> camera_status_;    //!< 状態

    float3 position_ = {0, 0, 0};    //!< カメラ座標
    float3 look_at_  = {0, 0, 1};    //!< 注視点
