...
public:
    ...
    [[deprecated("ComponentSpringArm::SetSpringArmOffset()は古い形式です。ComponentSpringArm::SetSpringArmLookOffsetを使用してください")]]
    ComponentSpringArmPtr SetSpringArmOffset(float3 offset)
    {
        return SetSpringArmLookOffset(offset);
    }

    ComponentSpringArmPtr SetSpringArmPositionOffset(float3 offset)
    {
        spring_arm_offset_ = offset;
        return std::dynamic_pointer_cast<ComponentSpringArm>(shared_from_this());
    }

    ComponentSpringArmPtr SetSpringArmLookOffset(float3 offset)
    {
        spring_arm_look_ = offset;
        return std::dynamic_pointer_cast<ComponentSpringArm>(shared_from_this());
    }
    ...
    {
        spring_arm_strong_ = std::clamp(strong, 0.1f, 1.0f);
        return std::dynamic_pointer_cast<ComponentSpringArm>(shared_from_this());
    }

    ComponentSpringArmPtr SetSpringArmReturn(float ret)
    {
        spring_arm_return_ = std::clamp(ret, 0.1f, 2.0f);
        return std::dynamic_pointer_cast<ComponentSpringArm>(shared_from_this());
    }
...
private:
    ...
    mutable bool   on_position_ = false;
    mutable float3 old_position_{};
    mutable float3 velocity_{};
