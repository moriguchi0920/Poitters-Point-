...
void ComponentCollisionCapsule::PostUpdate()
{
	...
	if(attach_node_ >= 0)
	{
		...
		{
			old_attach_node_matrix_ = attach_node_matrix_;
...
ComponentCollisionCapsule::HitInfo ComponentCollisionCapsule::IsHit(ComponentCollisionPtr col)
{
	...
	case ComponentCollision::CollisionType::BOX:
		return isHit(std::dynamic_pointer_cast<ComponentCollisionCapsule>(shared_from_this()), std::dynamic_pointer_cast<ComponentCollisionBox>(col));
		break;
	}
...
const matrix ComponentCollisionCapsule::GetOldWorldMatrix() const
{
	auto transform = collision_transform_;
	...
	if(attach_node_ >= 0)
	{
		...
		if(mdl)
		{
			transform = mul(transform, old_attach_node_matrix_);
		}
	}
	else
	{
		...
		if(cmp)
		{
			transform = mul(transform, cmp->GetOldWorldMatrix());
		}
	}
